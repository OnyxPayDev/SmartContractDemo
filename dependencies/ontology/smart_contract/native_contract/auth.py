...
class Auth(object):
    ...
    def assign_funcs_to_role(self, admin_identity: Identity, password: str, key_no: int, contract_address: str, role: str, function_name: list,
                             payer: Account, gas_limit: int, gas_price: int):
        contract_address = bytearray.fromhex(contract_address)
    ...
    def assign_ont_ids_to_role(self, admin_identity: Identity, password: str, key_no: int, contract_address: str, role: str, ont_ids: list,
                             payer: Account, gas_limit: int, gas_price: int):
        contract_address = bytearray.fromhex(contract_address)
        # param = {"contract_address": contract_address, "ontid": admin_identity.ont_id.encode('utf-8'), "role": role.encode('utf-8')}
        param = {"contract_address": contract_address, "ontid": admin_identity.ont_id.encode('utf-8'), "role": role.encode('utf-8')}
        ...
        param['key_no'] = key_no
        invoke_code = build_native_invoke_code(bytearray.fromhex(self.contract_address), bytes([0]), "assignOnxIDsToRole", param)
    ...
    def delegate(self, identity: Identity, password: str, key_no: int, contract_address: str, to_ont_id: str, role: str, period: int, level : int,
                             payer: Account, gas_limit: int, gas_price: int):
        contract_address = bytearray.fromhex(contract_address)
