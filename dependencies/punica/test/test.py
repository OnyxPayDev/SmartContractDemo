...
class Test:
    ...
    def get_abi_info(abi_dir_path: str):
        abi_dict = read_abi(os.path.abspath(os.path.join(abi_dir_path, os.pardir)), os.path.basename(abi_dir_path))
        ...
        if abi != '':
            ...
            if config != '':
                ...
                if not os.path.exists(config_path):
                    ...
                    os._exit(0)
            wallet, invoke_dict, *password = handle_invoke_config(project_dir_path, os.path.basename(config_path))
            print (invoke_dict)
    ...
    def generate_test_file(project_dir_path, abi_path):
        with open(abi_path, 'r') as f:
            ...
            with open(test_file_path, 'w') as f:
                ...
                f.writelines(
                    'abi_path = ' + '\'' + 'contracts/build/' + os.path.basename(abi_path) + '\'' + '\n')
                contract_address_tmp = dict_abi['hash'].replace('0x', '')
                contract_address = "".join(reversed([contract_address_tmp[i:i+2] for i in range(0, len(contract_address_tmp), 2)]))
                f.writelines('contract_address = bytes.fromhex(' + '\'' + contract_address + '\')' + '\n')
                f.writelines('wallet_path= ' + '\'' + 'wallet/wallet.json' + '\'' + '\n')
                f.writelines('\n')
                f.writelines('sdk = OntologySdk()' + '\n')
                f.writelines('sdk.set_rpc'+'(' + '\'' + 'http://127.0.0.1:20336' + '\'' + ')' + '\n')
