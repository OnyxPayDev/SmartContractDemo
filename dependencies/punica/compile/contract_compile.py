...
PYTHON_COMPILE_URL = 'https://smartxcompiler.ont.io/api/v1.0/python/compile'
...
class PunicaCompiler:
    ...
    @staticmethod
    def compile_contract(contract_path: str, local: bool = True, abi_save_path: str = '', avm_save_path: str = ''):
        if abi_save_path == '':
            split_path = os.path.split(contract_path)
            save_path = os.path.join(os.path.dirname(split_path[0]), 'build', split_path[1])
    ...
    def compile_contract_remote(contract_path: str):
        with open(contract_path, "r") as f:
            ...
            response = session.post(url, json=dict_payload, headers=header, timeout=timeout, verify=False)
            if response.status_code != 200:
                raise PunicaException(PunicaError.get_error(5000, 'Remote compile failed.'))
